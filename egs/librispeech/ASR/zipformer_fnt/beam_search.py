...
class Hypothesis:
    ...
    blank_decoder_state: Optional[Tuple[torch.Tensor, torch.Tensor]] = None
    vocab_decoder_state: Optional[Tuple[torch.Tensor, torch.Tensor]] = None
    @property
    def key(self) -> str:
...
def modified_beam_search(
    ...
    blank_id = model.blank_decoder.blank_id
    device = next(model.parameters()).device
    unk_id = getattr(model, "unk_id", blank_id)
    test_input = torch.tensor([[blank_id]], device=device, dtype=torch.int64)
    _, (blank_init_h, blank_init_c) = model.blank_decoder(
        test_input, None
    )
    blank_init_h = torch.zeros_like(blank_init_h)
    blank_init_c = torch.zeros_like(blank_init_c)

    _, (vocab_init_h, vocab_init_c) = model.vocab_decoder(
        test_input, None
    )
    vocab_init_h = torch.zeros_like(vocab_init_h)
    vocab_init_c = torch.zeros_like(vocab_init_c)
    ...
    for i in range(N):
        B[i].add(
            Hypothesis(
                ...
                timestamp=[],
                blank_decoder_state=(blank_init_h, blank_init_c),
                vocab_decoder_state=(vocab_init_h, vocab_init_c),
            )
        )
    ...
    for t, batch_size in enumerate(batch_size_list):
        ...
        decoder_input = torch.tensor(
            [hyp.ys[-1:] for hyps in A for hyp in hyps],
            device=device,
            dtype=torch.int64,
        )  # (num_hyps, context_size)

        # output of blank decoder
        blank_decoder_state = [
            hyp.blank_decoder_state
            for hyps in A
            for hyp in hyps
        ]
        blank_decoder_state_h = torch.cat(
            [item[0] for item in blank_decoder_state], dim=1
        )
        blank_decoder_state_c = torch.cat(
            [item[1] for item in blank_decoder_state], dim=1
        )
        blank_decoder_state = (
            blank_decoder_state_h,
            blank_decoder_state_c,
        )
        blank_decoder_out, blank_new_decoder_state = model.blank_decoder(
            decoder_input, blank_decoder_state
        )

        # output of vocab decoder
        vocab_decoder_state = [
            hyp.vocab_decoder_state
            for hyps in A
            for hyp in hyps
            if hyp.vocab_decoder_state is not None
        ]
        vocab_decoder_state_h = torch.cat(
            [item[0] for item in vocab_decoder_state], dim=1
        )
        vocab_decoder_state_c = torch.cat(
            [item[1] for item in vocab_decoder_state], dim=1
        )
        vocab_decoder_state = (
            vocab_decoder_state_h,
            vocab_decoder_state_c,
        )
        vocab_decoder_out, vocab_new_decoder_state = model.vocab_decoder(
            decoder_input, vocab_decoder_state
        )

        blank_decoder_out = blank_decoder_out.unsqueeze(1)
        vocab_decoder_out = vocab_decoder_out.unsqueeze(1)
        blank_decoder_out = model.joiner.blank_decoder_proj(blank_decoder_out)
        vocab_decoder_out = model.joiner.vocab_decoder_proj(vocab_decoder_out)
        # decoder_out is of shape (num_hyps, 1, 1, joiner_dim)
        hyp_index = torch.arange(len(decoder_input), device=device, dtype=torch.int32)
        ragge_hyp_index = k2.RaggedTensor(hyps_shape, hyp_index)
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                decoder_state_index = ragge_hyp_index[i][hyp_idx]
                ...
                if new_token not in (blank_id, unk_id):
                    ...
                    hyp_blank_decoder_state = (
                        blank_new_decoder_state[0][
                            :, decoder_state_index : decoder_state_index + 1
                        ],
                        blank_new_decoder_state[1][
                            :, decoder_state_index : decoder_state_index + 1
                        ],
                    )
                    hyp_vocab_decoder_state = (
                        vocab_new_decoder_state[0][
                            :, decoder_state_index : decoder_state_index + 1
                        ],
                        vocab_new_decoder_state[1][
                            :, decoder_state_index : decoder_state_index + 1
                        ],
                    )
                    ...
                    hyp_blank_decoder_state = hyp.blank_decoder_state
                    hyp_vocab_decoder_state = hyp.vocab_decoder_state
                ...
                new_hyp = Hypothesis(
                    ...
                    blank_decoder_state=hyp_blank_decoder_state,
                    vocab_decoder_state=hyp_vocab_decoder_state,
                )
                B[i].add(new_hyp)
    ...
    sorted_ans = [h.ys[1:] for h in best_hyps]
