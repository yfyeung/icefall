...
from train import get_model, add_model_arguments
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If True, ignore wandb and oom check.",
    )
...
def run(rank, world_size, args):
    ...
    setup_logger(f"{backup_path}/log")
    ...
    train_cuts = GigaSpeech.train_text()
    train_dl = GigaSpeech.train_text_dataloaders(train_cuts, is_distributed=(world_size>1))

    valid_cuts = GigaSpeech.dev_text()
    valid_dl = GigaSpeech.valid_text_dataloaders(valid_cuts, is_distributed=(world_size>1))

    # if not params.print_diagnostics and not params.debug:
...
def display_and_save_batch(
    ...
    y = sp.encode(batch, out_type=int)
