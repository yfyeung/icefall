...
def do_rnnt_pruning(
    am: torch.Tensor, blank: torch.Tensor,  lm: torch.Tensor, ranges: torch.Tensor
    ...
    return am_pruned, blank_pruned, lm_pruned
...
class AsrModel(nn.Module):
    def __init__(
        ...
        blank_decoder: Optional[nn.Module] = None,
        ...
        if use_transducer:
            ...
            assert blank_decoder is None
            assert vocab_decoder is None
    ...
    def forward_transducer(
        ...
        lm = self.simple_lm_proj(blank_decoder_out)
        ...
        am_pruned, blank_pruned, lm_pruned = do_rnnt_pruning(
            am=self.joiner.encoder_proj(encoder_out),
            blank=self.joiner.blank_decoder_proj(blank_decoder_out),
            lm=lm_probs,
            ranges=ranges,
        )
        ...
        logits, _ = self.joiner(am_pruned, blank_pruned, lm_pruned, project_input=False)
        ...
        return simple_loss, pruned_loss, lm_loss
    ...
    def forward(
        ...
        if self.use_transducer:
            # Compute transducer loss
            simple_loss, pruned_loss, lm_loss = self.forward_transducer(
        ...
        return simple_loss, pruned_loss, ctc_loss, lm_loss
